...
def test_workflow_builder_accessible_from_workflow():
    ...
    config: dict[Any, Any] = load_configuration_from_env(dict(environ))  # pyright: ignore[reportUnknownVariableType]

    assert (
        config.get("CONNECTIONS", {}).get("SERVICE_CONNECTION", {}).get("SETTINGS", {}).get("CLIENTID")
        == "test-client-id"
    )
    assert (
        config.get("CONNECTIONS", {}).get("SERVICE_CONNECTION", {}).get("SETTINGS", {}).get("CLIENTSECRET")
        == "test-client-secret"
    )
    assert config.get("bot_app_id") is None
